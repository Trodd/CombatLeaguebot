...
class DevPanel_Match(SafeView):
    ...
    @discord.ui.button(label="♻️ Reset Weekly Matches", style=discord.ButtonStyle.red, custom_id="dev:reset_weekly_matches")
...
class DevPanel_Score(SafeView):
    ...
    @discord.ui.button(label="❌ Clear Proposed Match", style=discord.ButtonStyle.primary, custom_id="dev:clear_match")
    async def clear_proposed(self, interaction, button):
        await self.generic_clear(interaction, "Match Proposed")

    @discord.ui.button(label="❌ Clear Proposed Score", style=discord.ButtonStyle.blurple, custom_id="dev:clear_score")
    async def clear_proposed_score(self, interaction, button):
        await self.generic_clear(interaction, "Scoring")

    @discord.ui.button(label="🏆 Undo Score For Match", style=discord.ButtonStyle.blurple, custom_id="dev:undo_score")
...
class DevPanel_System(SafeView):
    ...
    @discord.ui.button(label="♻️ Reload Views", style=discord.ButtonStyle.green, custom_id="dev:reload_view")
    async def reload_views(self, interaction, button):
        await interaction.response.defer(ephemeral=True)
        await self.bot.tree.sync()
        await interaction.followup.send("✅ Views reloaded.", ephemeral=True)

    @discord.ui.button(label="🔒 Lock Rosters", style=discord.ButtonStyle.red, custom_id="dev:lock_roster")
    ...
    @discord.ui.button(label="🔓 Unlock Rosters", style=discord.ButtonStyle.green, custom_id="dev:unlock_roster")
