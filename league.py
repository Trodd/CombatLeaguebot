...
teams_sheet = get_or_create_sheet(spreadsheet, "Teams", ["Team Name", "Player 1", "Player 2", "Player 3", "Player 4", "Player 5", "Player 6", "Status", "Co-Captain"])
...
proposed_scores_sheet = get_or_create_sheet(spreadsheet, "Proposed Scores", ["Match ID", "Team A", "Team B", "Proposer ID", "Proposed Date", "Channel ID", "Message ID", "Map Scores"])
...
async def auto_update_team_embeds(bot, teams_sheet):
    ...
    while not bot.is_closed():
        ...
        for row in team_rows:
            ...
            members = row[1:7]  # slots B–G (Captain + 5 others)
            mentions = []

            # 🔍 Captain (slot 1 only)
            captain_id = None
            captain = "N/A"
            if members and "(" in members[0] and ")" in members[0]:
                captain_id = members[0].split("(")[-1].split(")")[0]
                captain_member = channel.guild.get_member(int(captain_id))
                captain_display = captain_member.display_name if captain_member else "Unknown"
                captain = f"<@{captain_id}> ({captain_display})"
            elif members and members[0].strip():
                captain = members[0].strip()

            # 👥 Other players (slots 2–6 only)
            for idx, member in enumerate(members[1:], start=1):  # Skip captain
                if not member.strip():
                    continue
                ...
                if "(" in member and ")" in member:
                    ...
                    mention = f"<@{user_id}> ({display_name})"

                    # 🧢 Co-captain emoji
                    if idx == 1 and member_obj:  # Slot 2 is co-captain
                        co_captain_role_id = bot.config.get("co_captain_role_id")
                        if co_captain_role_id and any(role.id == co_captain_role_id for role in member_obj.roles):
                            mention += " 🧢"
                else:
            ...
            if len(members) > 0 and "(" in members[0] and ")" in members[0]:
                captain_id = members[0].split("(")[-1].split(")")[0]
                captain_member = channel.guild.get_member(int(captain_id))
                display_name = captain_member.display_name if captain_member else "Unknown"
                captain = f"<@{captain_id}> ({display_name})"
...
async def rehydrate_join_requests(bot):
    ...
    for entry in data:
        try:
            ...
            captain = guild.get_member(entry["captain_id"])
            invitee = guild.get_member(entry["invitee_id"])
...
async def on_ready():
            ...
            try:
                ...
                embed.add_field(
                    ...
                    value=(
                        ...
                        "👉 Go to the https://discord.com/channels/779349159852769310/1206105633389019217/1391543836416807153 to open one"
                    ),
                    inline=False
                )
    ...
    for idx, row in enumerate(score_rows, start=2):
        if len(row) < 8:
            print(f"⚠️ Skipping incomplete score row {row}")
            continue

        match_id, team1, team2, proposer_id, proposed_date, channel_id, message_id, map_scores_json = row

        # Parse saved map scores
        try:
            map_scores = json.loads(map_scores_json)
        except Exception as e:
            print(f"❗ Failed to load map_scores for {match_id}: {e}")
            continue

        if not channel_id or not message_id or not channel_id.isdigit() or not message_id.isdigit():
            print(f"⚠️ Skipping score {match_id}: Invalid channel/message ID")
            continue
        ...
        try:
            ...
            message = await channel.fetch_message(int(message_id))
            guild = channel.guild
            proposer = guild.get_member(int(proposer_id)) or discord.Object(id=int(proposer_id))  # fallback object if member left

            from command_buttons import ConfirmScoreView

            view = ConfirmScoreView(
                parent=bot.league_panel,
                match={"match_id": match_id, "team1": team1, "team2": team2},
            ...
            print(f"❌ Failed to rehydrate score confirmation for {match_id}: {e}")
