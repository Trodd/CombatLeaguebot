...
def safe_get_values(sheet, retries=3, delay=1.5):
    import time
    import gspread
    for attempt in range(retries):
        try:
            ...
            print(f"[⚠️] Sheet fetch error (attempt {attempt+1}): {e}")
...
def is_dev_override(user, config):
    dev_ids = config.get("dev_override_ids", [])
    return user.id in dev_ids or any(role.id in dev_ids for role in user.roles)

def get_captains_and_cocaps(guild, team_row):
    members = []

    # Column B = Captain
    try:
        raw = str(team_row[1]).strip()
        if "(" in raw and ")" in raw:
            user_id = int(raw.split("(")[-1].replace(")", "").strip())
        else:
            user_id = int(raw)
        ...
        print(f"[⚠️] Failed parsing captain from team_row[1]: {e}")

    # Column I = Co-captain (ID or Name (ID))
    try:
        if len(team_row) > 8:
            raw = str(team_row[8]).strip()
            if "(" in raw and ")" in raw:
                user_id = int(raw.split("(")[-1].replace(")", "").strip())
            elif raw.isdigit():
                user_id = int(raw)
            ...
            if user_id:
                member = guild.get_member(user_id)
                if isinstance(member, discord.Member) and member not in members:
                    members.append(member)
    except Exception as e:
        print(f"[⚠️] Failed parsing co-captain from team_row[8]: {e}")

    return members

def is_captain_or_cocap(user_id: str, member: discord.Member, team_row: list, co_captain_role_id: int):

    # Captain check from Column B
    if len(team_row) > 1 and f"({user_id})" in str(team_row[1]):
        return True

    # Co-captain check from Column I
    cocap_id_raw = team_row[8] if len(team_row) > 8 else ""
    try:
        if "(" in cocap_id_raw and ")" in cocap_id_raw:
            cocap_id = cocap_id_raw.split("(")[-1].replace(")", "").strip()
        else:
            cocap_id = cocap_id_raw.strip()
    except Exception:
        cocap_id = ""
    has_role = any(role.id == co_captain_role_id for role in member.roles)
    if cocap_id == user_id and has_role:
        return True
    return False
    ...
    async def accept(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not guild:
            await interaction.message.edit(
                content="❌ This join request is no longer valid. The server context is missing.",
                view=None
            )
            return
        # Failsafes: validate that player is still eligible
        players_sheet = self.parent_view.players_sheet
        teams_sheet = self.parent_view.teams_sheet
        sub_role = discord.utils.get(guild.roles, id=self.parent_view.bot.config.get("league_sub_role_id"))

        # 1. Check if still signed up
        if not any(row[0].strip() == str(self.invitee.id) for row in players_sheet.get_all_values()[1:]):
            await interaction.message.edit(
                content="❌ This join request is no longer valid. The player is no longer signed up.",
                view=None
            )
            return

        # 2. Check if player became a league sub
        if sub_role and sub_role in self.invitee.roles:
            await interaction.message.edit(
                content="❌ This join request is no longer valid. The player is now a League Sub.",
                view=None
            )
            return

        # 3. Check if already joined another team
        for row in teams_sheet.get_all_values()[1:]:
            for cell in row[1:7]:
                if f"({self.invitee.id})" in cell:
                    await interaction.message.edit(
                        content="❌ This join request is no longer valid. The player has already joined another team.",
                        view=None
                    )
                    return
        ...
        if not team_role:
            await interaction.message.edit(
                content="❌ This join request is no longer valid. The team role no longer exists.",
                view=None
            )
            return
        ...
        if already_on_team:
            await interaction.message.edit(
                content="❌ This join request is no longer valid. The player is already on another team.",
                view=None
            )
            return
...
class AcceptDenyMatchView(discord.ui.View):
            ...
            async def accept(self, interaction: discord.Interaction, button: discord.ui.Button):
                ...
                if is_dev_override(interaction.user, self.parent.config):
                ...
                for row in self.parent.teams_sheet.get_all_values()[1:]:
                    if any(user_id in cell for cell in row[1:] if cell):
                        user_team = row[0]
                        break

                if not user_team:
                    await safe_send(interaction, "❗ You are not on either team for this match.", ephemeral=True)
                    return

                # Determine which team the proposer is on
                proposer_team = None
                for row in self.parent.teams_sheet.get_all_values()[1:]:
                    if any(str(self.proposer_id) in cell for cell in row[1:] if cell):
                        proposer_team = row[0]
                        break

                # 🚫 Prevent *anyone* from the proposing team (not just proposer) from accepting
                if user_team == proposer_team:
                ...
                receiving_team = self.team_b if proposer_team == self.team_a else self.team_a

                # Confirm they are actually on the receiving team
                if user_team != receiving_team:
                    await safe_send(interaction, "❗ Only the receiving team's captain or co-captain can accept this proposal.", ephemeral=True)
                    return

                # Confirm they are captain or co-captain of the receiving team
                team_row = next((r for r in self.parent.teams_sheet.get_all_values()[1:] if r[0] == receiving_team), [])
                if not is_captain_or_cocap(user_id, interaction.user, team_row, co_captain_role_id):
                    await safe_send(interaction, "❗ Only the captain or co-captain of the receiving team may accept this proposal.", ephemeral=True)
                    return


                # ✅ All checks passed — now disable buttons
                await interaction.response.defer(ephemeral=True)
                if getattr(self, "already_responded", False):
                    await interaction.followup.send("❗ This proposal has already been responded to.", ephemeral=True)
                    return
                ...
                if match_channel:
                    ...
                    unique_mentions = list(dict.fromkeys(all_mentions))

                    await match_channel.send(
                        content=f"{' '.join(unique_mentions)}",
                        embed=embed
                    )
            ...
            async def decline(self, interaction: discord.Interaction, button: discord.ui.Button):
                ...
                # Block the proposer’s own team from accepting
                proposer_team = None
                for row in self.parent.teams_sheet.get_all_values()[1:]:
                    if any(str(self.proposer_id) in cell for cell in row[1:] if cell):
                        proposer_team = row[0]
...
class ConfirmScoreView(discord.ui.View):
    def __init__(self, parent, match, map_scores, proposer, proposer_id, private_channel=None):
        ...
        self.team_a = match.get("team1")
        self.team_b = match.get("team2")
    ...
    async def accept(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if is_dev_override(interaction.user, self.parent.bot.config):
            await interaction.response.defer(ephemeral=True)
        ...
        if not user_team:
            await self.safe_send(interaction, "❗ You are not on either team for this match.")
            return

        # Block the proposer’s own team from confirming their own score
        proposer_team = None
        for row in self.parent.teams_sheet.get_all_values()[1:]:
            if any(str(self.proposer_id) in cell for cell in row[1:] if cell):
                proposer_team = row[0]
        ...
        # 🚨 Only defer once the user is validated
        await interaction.response.defer(ephemeral=True)
        ...
        if winner != "Tie":
            ...
            default_rating = self.parent.bot.config.get("default_player_rating", 800)
            ...
            # ✅ PLAYER STATS UPDATE
            def update_player_stats(sheet, user_id, username, won, elo_win, elo_loss, default_rating):
                values = sheet.get_all_values()
                existing = {row[1]: row for row in values[1:]}  # key = user_id
                elo_change = elo_win if won else elo_loss

                if user_id in existing:
                    row = existing[user_id]
                    rating = int(row[2]) if row[2].isdigit() else default_rating
                    ...
                    rating = default_rating + elo_change
                    sheet.append_row([username, user_id, str(rating), "1" if won else "0", "0" if won else "1", "1"])
            ...
            def credit_team_players(team_name, won):
                ...
                for cell in row[1:]:
                    if "(" in cell and ")" in cell:
                        ...
                        update_player_stats(self.parent.bot.player_leaderboard_sheet, user_id, username, won, elo_win, elo_loss, default_rating)
    ...
    async def deny(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if is_dev_override(interaction.user, self.parent.bot.config):
            if self.already_responded:
                return
            self.already_responded = True
            await self.safe_send(interaction, "❌ Score denied by override.")
            for item in self.children:
                item.disabled = True
            await interaction.message.edit(view=self)
            # then do the cleanup logic
            return
...
# -------------------- CREATE TEAM --------------------
        ...
        async def create_private_channel(guild, category_id, channel_name, members):
            ...
            for member in members:
                try:
                    ...
                    print(f"[💥] Failed to add overwrite for {member} ({type(member)}): {e}")

            # ✅ Fix: handle dev_override_ids as list
            dev_role_ids = self.bot.config.get("dev_override_ids", [])
            if not isinstance(dev_role_ids, list):
                dev_role_ids = [dev_role_ids]

            for role_id in dev_role_ids:
            ...
            class SubmitButton(discord.ui.Button):
                ...
                async def callback(self, interaction: discord.Interaction):
                    ...
                    try:
                        ...
                        for team_name in [self.parent_view.team_a, self.parent_view.team_b]:
                            team_row = next(
                                (r for r in self.parent_view.parent.teams_sheet.get_all_values()[1:] if r[0] == team_name), None
                            )
                            if team_row:
                                members = get_captains_and_cocaps(guild, team_row)
                                members_to_add.extend(m for m in members if isinstance(m, discord.Member))

                        private_channel = await create_private_channel(
                            guild,
                            fallback_id,
                            f"proposed-match-{self.parent_view.team_a}-vs-{self.parent_view.team_b}",
                            members_to_add  # ✅ guaranteed flat and clean
                        )
                    except Exception as e:
    ...
    async def propose_score(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        async def create_private_channel(guild, category_id, channel_name, members):
            ...
            # ✅ Add override roles (e.g. devs) from config
            dev_role_ids = self.bot.config.get("dev_override_ids", [])
        ...
        class MapScoreView(discord.ui.View):
                ...
                async def callback(self, interaction: discord.Interaction):
                    ...
                    if not all(k in self.view_obj.map_scores.get(current, {}) for k in ["gamemode", "team1_score", "team2_score"]):
                        try:
                            ...
                            print("❗ Cannot edit message: Message expired or deleted.")
                        return
                    ...
                    if current == 2:
                        ...
                        try:
                            if a_wins == 1 and b_wins == 1:
                                ...
                                text = self.view_obj.status_text()
                                view = self.view_obj
                            else:
                                text = self.view_obj.status_text() + "\n\n__**Note:**__ Use the dropdowns below to add league subs if any were used. Leave as None if no subs played.\n\nPlease confirm and submit your score proposal:"
                                view = ConfirmProposalView(self.view_obj)

                            await self.view_obj.message.edit(content=text, view=view)
                        except discord.NotFound:
                            print("❗ Cannot edit message: Message expired or deleted.")
                        return

                    # 🏁 Final confirmation (Map 3 complete)
                    if current == 3:
                        try:
                            text = self.view_obj.status_text() + "\n\n__**Note:**__ Use the dropdowns below to add league subs if any were used. Leave as None if no subs played.\n\nPlease confirm and submit your score proposal:"
                            view = ConfirmProposalView(self.view_obj)
            ...
            async def submit(self, interaction: discord.Interaction):
                ...
                for team_name in [team1, team2]:
                    team_row = next((r for r in self.parent.teams_sheet.get_all_values()[1:] if r[0] == team_name), None)
                    if team_row:
                        members = get_captains_and_cocaps(guild, team_row)
                        members_to_add.extend(m for m in members if isinstance(m, discord.Member))
                ...
                try:
                    ...
                    new_row = [
                        ...
                        str(private_channel.id),
                        str(msg.id),
                        json.dumps(self.map_scores)
                    ]
                    rows = safe_get_values(sheet)
        ...
        class MatchSelectView(discord.ui.View):
            ...
            async def match_selected(self, interaction: discord.Interaction):
                ...
                try:
                    ...
                    print("❗ Could not fetch original_response — possibly expired interaction.")
    ...
    @discord.ui.button(label="🔍 Ping Eligible Subs", style=discord.ButtonStyle.green, custom_id="league:find_subs", row=3)
    async def find_subs(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not is_captain_or_cocap(user_id, interaction.user, team_row, co_captain_role_id):
            await interaction.response.send_message("❗ Only captains or co-captains can ping eligible subs.", ephemeral=True)
            return

        team_name = team_row[0]
        team_elo = next((int(r[1]) for r in self.leaderboard_sheet.get_all_values()[1:] if r[0].strip() == team_name), None)
        if not team_elo:
            await interaction.response.send_message("❗ Could not find your team's ELO.", ephemeral=True)
            return

        scheduled_matches = self.spreadsheet.worksheet("Match Scheduled").get_all_values()[1:]
        team_matches = []
        for row in scheduled_matches:
            if len(row) >= 4 and (row[1] == team_name or row[2] == team_name):
                opponent = row[2] if row[1] == team_name else row[1]
                match_time = row[3]
                team_matches.append((row[0], opponent, match_time))  # match_id, opponent, date

        if not team_matches:
            await interaction.response.send_message("❗ No upcoming matches found for your team.", ephemeral=True)
            return

        class MatchSelectView(discord.ui.View):
            def __init__(self, parent, matches, team_name, team_elo):
                ...
                options = [
                    discord.SelectOption(
                        label=f"vs {opponent} • {match_time}",
                        value=f"{match_id}|{opponent}|{match_time}"
                    ) for match_id, opponent, match_time in matches
                ]

                select = discord.ui.Select(placeholder="Select match to ping subs for", options=options)
                select.callback = self.on_select
                self.add_item(select)

            async def on_select(self, interaction: discord.Interaction):
                selected = interaction.data['values'][0]
                match_id, opponent, time_raw = selected.split("|")

                # Extract raw timestamp from <t:...:f>
                match = re.search(r"<t:(\d+):f>", time_raw)
                if match:
                    time_str = f"<t:{match.group(1)}:f>"
                else:
                    time_str = time_raw  # fallback

                players_sheet = self.parent.players_sheet.get_all_values()[1:]
                elo_sheet = self.parent.bot.player_leaderboard_sheet.get_all_values()[1:]
                lower, upper = self.team_elo - 100, self.team_elo + 100

                team_players = {
                    cell.split("(")[-1].split(")")[0]
                    for row in self.parent.teams_sheet.get_all_values()[1:]
                ...
                for row in elo_sheet:
                    try:
                        ...
                        if not player_row:
                            continue
                        eligible.append((name, uid, rating, row[3], row[4]))
                    except:
                        continue

                eligible = sorted(eligible, key=lambda r: r[2], reverse=True)[:24]
                if not eligible:
                    await interaction.response.send_message("❗ No eligible subs found in ELO range.", ephemeral=True)
                    return

                mentions = [f"<@{e[1]}>" for e in eligible]
                ping_channel = interaction.guild.get_channel(int(self.parent.config["sub_ping_channel_id"]))

                if not ping_channel:
                    await interaction.response.send_message("❗ Sub ping channel not found.", ephemeral=True)
                    return

                msg = (
                    f"📢 **{self.team_name}** is looking for a sub for their match vs **{opponent}** "
                    f"on **{time_str}**\n\n"
                    f"Eligible players:\n" + " ".join(mentions)
                )
                await ping_channel.send(msg)
                await interaction.response.send_message("✅ Sub ping sent.", ephemeral=True)

        view = MatchSelectView(self, team_matches, team_name, team_elo)
        await interaction.response.send_message("📅 Choose your match to ping subs for:", view=view, ephemeral=True)
...
#                    self.bot.loop.create_task(view.expire_notice_dm())
        ...
        for idx, row in enumerate(self.teams_sheet.get_all_values(), 1):
            ...
            for col, cell in enumerate(members, start=2):  # B-G = cols 2–7
                if extract_user_id(cell) == user_id:
                    ...
                    # Remove player and preserve layout
                    updated_row = row.copy()

                    # Remove the player from their slot (columns 2–7)
                    if col < len(updated_row):
                        updated_row[col - 1] = ""  # `col` is 2–7; subtract 1 for list index

                    # Pad the row to at least 9 columns: Team name, 6 players, status, co-captain
                    while len(updated_row) < 9:
                        updated_row.append("")

                    # Write updated row back to sheet
                    col_end = chr(ord('A') + len(updated_row) - 1)
                    self.teams_sheet.update(f"A{idx}:{col_end}{idx}", [updated_row])

                    # 🧼 Clear Co-Captain column (I) if user was listed there
                    try:
                        cocap_cell = self.teams_sheet.cell(idx, 9).value  # Column I
                        if cocap_cell and f"({user_id})" in cocap_cell:
                            self.teams_sheet.update_cell(idx, 9, "")
                    except Exception as e:
                        print(f"⚠️ Failed to clear Co-Captain column during leave: {e}")
                ...
                class PromoteSelect(discord.ui.View):
                    ...
                    async def promote(self, select_interaction: discord.Interaction):
                        ...
                        if self.role_type == "captain":
                            ...
                            captain_slot = row[1]
                            promoted_slot_index = None

                            for i in range(1, 7):
                                if extract_user_id(row[i]) == new_user_id:
                                    promoted_slot_index = i
                                    break

                            if promoted_slot_index is not None:
                                row[1] = row[promoted_slot_index]
                                row[promoted_slot_index] = captain_slot
                            else:
                                # Fallback: overwrite if not found
                                row[1] = f"{new_member.display_name} ({new_member.id})"

                            self.parent.teams_sheet.update(f"A{self.team_idx}:G{self.team_idx}", [row[:7]])
                            # 🧼 Clear Co-Captain column (I) if the promoted user was listed there
                            try:
                                cocap_cell = self.parent.teams_sheet.cell(self.team_idx, 9).value  # Column I = 9
                                if cocap_cell and f"({new_user_id})" in cocap_cell:
                                    self.parent.teams_sheet.update_cell(self.team_idx, 9, "")
                            except Exception as e:
                                print(f"⚠️ Failed to clear Co-Captain column after Captain promotion: {e}")
                            ...
                            # Swap the promoted user with Player 2 (slot index 2)
                            player2_slot = row[2]
                            promoted_slot_index = None

                            # Find where the promoted user currently is
                            for i in range(1, 7):
                                if extract_user_id(row[i]) == new_user_id:
                                    promoted_slot_index = i
                                    break
                            ...
                            if promoted_slot_index is not None:
                                # Swap
                                row[2] = row[promoted_slot_index]
                                row[promoted_slot_index] = player2_slot
                            else:
                                # Fallback: just overwrite Player 2
                                row[2] = f"{new_member.display_name} ({new_member.id})"

                            self.parent.teams_sheet.update(f"A{self.team_idx}:G{self.team_idx}", [row[:7]])
                            # 🆕 Update Co-Captain column (column I → column 9) with name and ID format
                            try:
                                new_value = f"{new_member.display_name} ({new_member.id})"

                                # Clear old co-captain if it's the same
                                current_value = self.parent.teams_sheet.cell(self.team_idx, 9).value
                                if current_value and old_cocap_id and f"({old_cocap_id})" in current_value:
                                    self.parent.teams_sheet.update_cell(self.team_idx, 9, "")

                                # Set new co-captain
                                self.parent.teams_sheet.update_cell(self.team_idx, 9, new_value)
                            except Exception as e:
                                print(f"⚠️ Failed to update Co-Captain column: {e}")
        ...
        for team_row in self.teams_sheet.get_all_values()[1:]:
            ...
            if authorized:
                class DisbandModal(discord.ui.Modal, title="Confirm Disband"):
                    def __init__(self, parent_view, bot, team_name, row_index, cached_team_row):
                        super().__init__()
                        self.parent_view = parent_view
                        self.bot = bot
                        self.team_name = team_name
                        self.row_index = row_index
                        self.cached_team_row = cached_team_row
                    ...
                    async def on_submit(self, modal_interaction: discord.Interaction):
                        ...
                        try:
                            ...
                            print("⚠️ Interaction expired before deferral.")
                            return
                        ...
                        try:
                            ...
                            print(f"⚠️ Could not remove Co-Captain role: {e}")

                        # 🧼 Remove from Teams sheet
                        try:
                            self.parent_view.teams_sheet.delete_rows(self.row_index)
                        except Exception as e:
                            print(f"⚠️ Could not delete team row: {e}")
                        ...
                        # ✅ Follow-up message
                        try:
                            await modal_interaction.followup.send("✅ Team disbanded successfully.", ephemeral=True)
                        except Exception as e:
                            print(f"⚠️ Could not send disband success message: {e}")

                        # 📢 Public notification
                        try:
                            await self.parent_view.send_notification(
                                f"💥 **{self.team_name}** has been disbanded.\n{mention_text}"
                            )
                        except Exception as e:
                            print(f"❗ Failed to send disband notification: {e}")

                cached_row = team_row.copy()
                row_index = self.teams_sheet.get_all_values().index(team_row) + 1
                await interaction.response.send_modal(
                    DisbandModal(self, self.bot, team_name, row_index, cached_row)
                )
                return
        ...
        class KickView(discord.ui.View):
            ...
            async def kick_selected(self, select_interaction: discord.Interaction):
                ...
                try:
                    ...
                    updated_row = self.original_row.copy()

                    # Remove selected player from slots 2–7 (index 1 to 6)
                    for idx in range(1, 7):
                        if idx < len(updated_row) and updated_row[idx].strip() == selected.strip():
                            updated_row[idx] = ""

                    # Ensure at least 9 columns: [0] = team name, [1-6] = players, [7] = status, [8] = co-captain
                    while len(updated_row) < 9:
                        updated_row.append("")
                    ...
                    # 🧼 Clear Co-Captain column (I) if this was the co-captain
                    try:
                        cocap_cell = self.parent.teams_sheet.cell(self.row_index, 9).value  # column I
                        if cocap_cell and f"({kicked_user_id})" in cocap_cell:
                            self.parent.teams_sheet.update_cell(self.row_index, 9, "")
                    except Exception as e:
                        print(f"⚠️ Failed to clear co-captain column during kick: {e}")
    ...
    async def coin_flip(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        co_cap_role_id = self.config.get("co_captain_role_id")
        if not any(role.id in (cap_role_id, co_cap_role_id) for role in user.roles):
