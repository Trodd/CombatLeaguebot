...
# -------------------- CREATE TEAM --------------------
    ...
    @discord.ui.button(label="🏆 Propose Score", style=discord.ButtonStyle.green, custom_id="propose_score", row=2)
    ...
    @discord.ui.button(label="🚪 Leave Team", style=discord.ButtonStyle.gray, custom_id="league:leave_team", row=1)
    ...
    async def disband_team(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        for team_row in self.teams_sheet.get_all_values()[1:]:
            ...
            if authorized:
                # ✅ Show disband modal for confirmation
                class DisbandModal(discord.ui.Modal, title="Confirm Disband"):
                    def __init__(self, parent_view, bot, team_name, row_index):
                        ...
                        # Dummy input just for confirmation
                    ...
                    async def on_submit(self, modal_interaction: discord.Interaction):
                        ...
                        team_members = team_row[1:]  # skip team name
                        mentions = []
                        for cell in team_members:
                ...
                await interaction.response.send_modal(DisbandModal(self, self.bot, team_name, self.teams_sheet.get_all_values().index(team_row)+1))
                return

        # ❌ No team found or not authorized
    ...
    async def kick_player(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not team_row:
            await interaction.response.send_message("❗ You must be a team captain or to kick a player.", ephemeral=True)
            return
