...
class LeaguePanel(View):
    def __init__(self, bot, spreadsheet, players_sheet, teams_sheet, matches_sheet, scoring_sheet, leaderboard_sheet, proposed_sheet, scheduled_sheet, proposed_scores_sheet, weekly_matches_sheet, challenge_sheet, send_to_channel, send_notification, DEV_OVERRIDE_IDS):
...
# -------------------- CREATE TEAM --------------------
        ...
        class AcceptDenyMatchView(discord.ui.View):
            ...
            async def accept(self, interaction: discord.Interaction, button: discord.ui.Button):
                ...
                if interaction.user.id == int(self.proposer_id):
                    await interaction.followup.send("❗ Only the opponent captain may respond to this proposal.", ephemeral=True)
                    return
            ...
            async def submit(self, interaction: discord.Interaction, button: discord.ui.Button):
                ...
                if captain:
                    ...
                    if private_channel:
                        ...
                        self.parent.bot.add_view(view, message_id=msg.id)  # ✅ persistent button

                        # Store channel_id and message_id in the proposed_sheet
                        rows = self.parent.proposed_sheet.get_all_values()
                        for i, row in enumerate(rows[1:], start=2):
                            if row[0] == match_id:
                                self.parent.proposed_sheet.update_cell(i, 6, str(private_channel.id))  # Channel ID
                                self.parent.proposed_sheet.update_cell(i, 7, str(msg.id))              # Message ID
                                break
    ...
    async def propose_score(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        class ConfirmScoreView(discord.ui.View):
            ...
            async def accept(self, interaction: discord.Interaction, button: discord.ui.Button):
                ...
                if interaction.user.id == self.proposer.id:
                    await interaction.followup.send("❗ Only the opposing captain may confirm or deny this score.", ephemeral=True)
                    return
            ...
            async def deny(self, interaction: discord.Interaction, button: discord.ui.Button):
                ...
                if interaction.user.id == self.proposer.id:
                    await safe_send(interaction, "❗ Only the opposing captain may confirm or deny this score.", ephemeral=True)
                    return
                await safe_send(interaction, "❌ Scores denied.")
            ...
            async def submit(self, interaction: discord.Interaction, button: discord.ui.Button):
                ...
                if opponent_captain:
                    ...
                    if private_channel:
                        ...
                        # ✅ Save to Proposed Scores sheet
                        self.parent.proposed_scores_sheet.append_row([
                            self.match["match_id"],
                            self.match["team1"],
                            self.match["team2"],
                            str(interaction.user.id),
        ...
        for match in scheduled_matches:
            ...
            if (team1_role and user_id in [str(m.id) for m in team1_role.members]):
                proposer_team, opponent_team = team1, team2
            elif (team2_role and user_id in [str(m.id) for m in team2_role.members]):
            ...
            matches.append({
                "match_id": match_id,
                "match_type": "weekly",
                "team1": proposer_team,  # always assign the proposer's team as team1
                "team2": opponent_team,
                "date": date,
            ...
            if team[1] == username_id:
                ...
                class PromoteSelect(discord.ui.View):
                    ...
                    async def promote(self, select_interaction):
                        ...
                        # Replace captain cell (index 1) with new captain
                        # Replace new captain's old spot with old captain
                        for i in range(1, 7):
                            if row[i] == new_captain_str:
                                row[i] = old_captain_str
                                break

                        row[1] = new_captain_str  # index 1 is always captain
